...
#define LAUNCH_ACCEL_THRESHOLD_MS2 29.43f
...
static void record_fast_packet() {
    FastFlightPacket pkt;

    pkt.packet_id = 0x01;
    pkt.timestamp_ms = millis();
    pkt.mpu = mpuData; 
    pkt.bno = bnoData;
    pkt.transducer = transducerData;

    pkt.checksum = crc16((uint8_t*)&pkt, sizeof(FastFlightPacket) - sizeof(uint16_t));

    page_buf_write((uint8_t*)&pkt, sizeof(FastFlightPacket));
}

static void record_slow_packet() {
    SlowFlightPacket pkt;
    ...
    pkt.checksum = crc16((uint8_t*)&pkt, sizeof(SlowFlightPacket) - sizeof(uint16_t));

    page_buf_write((uint8_t*)&pkt, sizeof(SlowFlightPacket));
}
...
void flight_computer_update() {
    ...
    case STATE_PAD:
        ...
        if (now - last_fast_sample >= SAMPLE_INTERVAL_MS) { 
